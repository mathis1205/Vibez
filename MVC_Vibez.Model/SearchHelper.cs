...
public class SearchHelper
{
    ...
    public static async Task<Welcome> SearchAll(string searchWord)
    {
        ...
        if (response.IsSuccessStatusCode)
            return JsonConvert.DeserializeObject<Welcome>(await response.Content.ReadAsStringAsync())!;
        throw new Exception(
            $"Failed to search for '{searchWord}'. Status code: {response.StatusCode}, Reason: {response.ReasonPhrase}");
    }
    ...
    public static async Task<List<PlaylistsItem>> GetRandomPlaylistsAsync(int count)
    {
        ...
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.access_token);
        var response =
            await _httpClient.GetAsync($"https://api.spotify.com/v1/browse/featured-playlists?limit={count}");
